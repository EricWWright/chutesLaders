...
class Player(object):
    ...
    def move(self):
        """Decides if the player has landed on the 100th space."""
        roll = self.roll()
    ...
    def win(self):
        """Calculates if a player / user has won the game."""
        if self.position == 99:
            return self.token
        else:
            return None
...
class Board(object):
    ...
    def __init__(self):
        """Attribute for the board object."""
        self.board = []
    
    def create_space(self):
        """Creates empty spaces on the board and sets chutes and ladders."""
        esp = Space(0)
        for i in range(100):
...
class Space(object):
    ...
    def __init__(self,move):
        """Attribute for space object."""
        self.move = move
    
    def move_player(self,player,board):
        """Moves the player piece to its new position."""
        oldpos = player.position
        player.position = player.position + self.move
        if self.move > 0:
            print("this space has a lader "+ player.token + " moves up", self.move, "spaces")
            input("press enter to use the lader")
        elif self.move < 0:
...
def ask_num():
    ...
    while True:
        num_users = int(input("How many players will be playing? (2-4) "))
        if num_users == 1:
            print("The minimum number of players you can have is 2")
        elif num_users > 4:
            print("The maximum number of players you can have is 4")
        elif num_users == 0:
...
def switch_turn(num_players, turn):
    """Changes the turn from user to user."""
    turn = turn
...
def winner_grats():
    """Congradulates the player who wins."""
    print("Congradulations! You Win!")

def main():
    """Initializes the game for the users."""
    num_players = ask_num()
    players = []
    turn = 0
    winner = None
    ...
    while not winner:
        print(players[turn].name + " token " + players[turn].token + " it is your turn")
